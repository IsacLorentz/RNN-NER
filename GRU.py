...
class GRUCellV2(nn.Module):
    ...
    def forward(self, x, h):
        ...
        gateX = torch.mm(x, self.w_ih.t()) + self.b_ih  # (5,60) , (60)
        gateH = torch.mm(h, self.w_hh.t()) + self.b_hh  # (5,60), (60)

        inputReset, inputUpdate, inputNew = gateX.chunk(3, 1)
        hiddenReset, hiddenUpdate, hiddenNew = gateH.chunk(3, 1)

        resetGate = torch.sigmoid(inputReset + hiddenReset)
        updateGate = torch.sigmoid(inputUpdate + hiddenUpdate)
        tentNew = torch.tanh(inputNew + (resetGate * hiddenNew))

        h_t = tentNew + updateGate * (h - tentNew)
...
class GRU2(nn.Module):
    ...
    def __init__(
        ...
        self.fw = GRUCellV2(
            input_size, hidden_size, activation=activation
        )  # forward cell
    ...
    def forward(self, x):
        ...
        h_fw = torch.zeros(B, self.hidden_size)
        for t in range(T):
        ...
        if not self.bidirectional:
            ...
            outputsBW = torch.zeros((B, T, self.hidden_size))
            h_b = torch.zeros(B, self.hidden_size)
            for t in range(T - 1, -1, -1):
                h_b = self.bw(x[:, t, :], h_b)
                outputsBW[:, t, :] = h_b

            return (torch.cat((outputs, outputsBW), 2), h_fw, h_b)
...
if __name__ == "__main__":
    ...
    print(
        "Same hidden states of the forward cell?\t\t{}".format(
    ...
    print(
        "Same hidden states of the backward cell?\t{}".format(
