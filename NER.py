...
def load_glove_embeddings(
    embedding_file, padding_idx=0, padding_word=PADDING_WORD, unknown_word=UNKNOWN_WORD
):
    """
...
class NERDataset(Dataset):
    ...
    def __init__(self, filename):
        reader = csv.reader(
            codecs.open(filename, encoding="ascii", errors="ignore"), delimiter=","
        )
...
class PadSequence:
    ...
    def __call__(self, batch, pad_data=PADDING_WORD, pad_labels=0):
        ...
        padded_data = [
            [b[i] if i < len(b) else pad_data for i in range(max_len)]
            for b in batch_data
        ]
        padded_labels = [
            [l[i] if i < len(l) else pad_labels for i in range(max_len)]
...
class NERClassifier(nn.Module):
    def __init__(
        ...
        self.word_emb.weight = nn.Parameter(
            torch.from_numpy(embeddings), requires_grad=False
        )
    ...
    def forward(self, x):
        ...
        for i in range(B):
            for j in range(T):
                ...
                maxWordLen = (
                    wordLen
                    if (wordLen > maxWordLen and word != self.padding_word)
                    else maxWordLen
                )

        charsIdxsLists = []
        # create list of char indices for the embedding
        for i in range(B):
            for j in range(T):
                word = x[i][j]
                charList = list(word) if word != self.padding_word else []
                charIdx = [self.c2i[char] for char in charList]
                l = len(charList)
                charIdx.extend([0 for _ in range(maxWordLen - l)])
                charsIdxsLists.append(charIdx)

        
        embeddedChars = self.char_emb(torch.LongTensor(charsIdxsLists)) # chars idx tensor tensor: (B x T, max word length)   
        # embedded chars : (B x T, max word length, character embedding size)
        
        # get and reshape character level word vector
        outputs, h_fw, h_bw = self.char_birnn(embeddedChars) # h: (B x T, char hidden size)
        
        hiddens = torch.cat((h_fw, h_bw), 1)
        hiddens = torch.reshape(hiddens, (B, T, 2 * self.char_hidden_size)) # resize in order to concatinate with word level tensor

        # get word indices for the embedding
        wordIdxsList = []
        for sentence in x:
            wordIdxs = [self.w2i[word] if word in self.w2i else 1 for word in sentence]
            wordIdxsList.append(wordIdxs)

        embeddedWords = self.word_emb(torch.LongTensor(wordIdxsList)) # words idx tensor: (B x T)
        # embedded words : words idx tensor, embedding size (50)
        
        # concatentae glove vectors witch character level word vectors
        out = torch.cat((embeddedWords, hiddens), 2) # (B,T, word embedding size (50) + 2 x char hidden size)

        outputs, _, _ = self.word_birnn(out) #(B, T, 2 x word embedding size (50) + 2 x char hidden size)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-tr",
        "--train",
        default="data/ner_training.csv",
        help="A comma-separated training file",
    )
    parser.add_argument(
        "-t", "--test", default="data/ner_test.csv", help="A comma-separated test file"
    )
    parser.add_argument(
        "-wv",
        "--word-vectors",
    ...
    training_loader = DataLoader(
        training_data, batch_size=128, collate_fn=PadSequence()
    )
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in tqdm(training_loader, desc="Epoch {}".format(epoch + 1)):
            ...
            loss = criterion(
                logits.reshape(-1, logits_shape[2]), torch.tensor(y).reshape(-1,)
            )
            loss.backward()
